...
func _create_containers() -> void:
	...
	main_container.set_anchors_and_offsets_preset(PRESET_TOP_LEFT, PRESET_MODE_MINSIZE)
...
func _fit_camera_to_view(use_size: Vector2 = Vector2.ZERO) -> void:
	if not camera or not is_inside_tree():
		return
	var view_size := use_size
	if view_size.x < 10 or view_size.y < 10:
		view_size = get_rect().size
	if view_size.x < 10 or view_size.y < 10:
		# Try parent size as fallback
		var p = get_parent_control()
		if p:
			view_size = p.size
	if view_size.x < 10 or view_size.y < 10:
		call_deferred("_fit_camera_to_view")
		return
	# Calculate zoom to fit the whole tree with some margin
	var margin = 1.2
	var zoom_x = view_size.x / (_tree_data.size.x * margin)
	var zoom_y = view_size.y / (_tree_data.size.y * margin)
	var fit_zoom = min(zoom_x, zoom_y)
	fit_zoom = clamp(fit_zoom, 0.05, 1.0)
	camera._zoom = fit_zoom
	main_container.scale = Vector2(fit_zoom, fit_zoom)
	# Center the tree in the viewport using position/scale
	var tree_center = _tree_data.size * 0.5
	var pos = view_size * 0.5 - tree_center * fit_zoom
	main_container.position = pos
	print("CAMERA: view_size=%s tree_size=%s zoom=%.3f tree_center=%s pos=%s" % [str(view_size), str(_tree_data.size), fit_zoom, str(tree_center), str(pos)])
	# Apply clamp for correct bounds
	if camera.has_method("_clamp"):
		camera._clamp()
