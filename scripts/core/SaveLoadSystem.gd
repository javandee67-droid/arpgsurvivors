...
static func _serialize_player(player: Node) -> Dictionary:
	...
	var data := {
		"level": player.level_system.level if "level_system" in player else 1,
		"xp": player.level_system.xp if "level_system" in player else 0,
		"xp_to_next": player.level_system.xp_to_next_level if "level_system" in player else 100,
		"position": {
			"x": player.global_position.x,
			"y": player.global_position.y,
		},
		"health": player.health.current if "health" in player else 100,
		"max_health": player.health.max_value if "health" in player else 100,
	}
		...
		for item in inv.items:
			if item:
				items.append({
					...
					"name": item.name,
					"rarity": item.rarity,
					"stack_count": item.stack_count if "stack_count" in item else 1,
				})
...
static func _serialize_passive_tree(player: Node) -> Dictionary:
	...
	if "_passive_nodes_owned" in player:
...
static func apply_save_data(player: Node, data: Dictionary) -> void:
	...
	if not player_data.is_empty():
		if "level_system" in player and player_data.has("level"):
		...
		if "health" in player and player_data.has("health"):
